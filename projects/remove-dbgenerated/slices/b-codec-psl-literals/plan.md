...
- **Local gates missed two CI-only checks.** `pnpm lint:agent` does not run `lint:throws` (the bare-throw ratchet) or `check:upgrade-coverage --mode pr` (a diff under `packages/3-extensions/` needs a pending upgrade-instruction fragment). Both failed on CI and were fixed after the PR opened. Lesson: a slice gate list must include every step CI's Lint job runs, and any slice that changes a public interface extension authors implement must plan the upgrade fragment as a dispatch item.
