...
Stop guessing. The foreign-key path for the same relation already leaves the target table unset when `relation.tableName` is undefined and defers resolution to the planner, which has the remote contract (ADR 226). The relation node path does the same: for a cross-space relation with no `tableName`, `toTable` and `on.childTable` are left undefined rather than fabricated. The only reader of those fields in `build-contract.ts` already skips cross-space relations, so no consumer changes behaviour.
