 ...
 *
 * A literal the column's codec has no PSL spelling for is refused, because
 * every spelling that would parse reads back as a different value.
 */
export function printColumnDefault(input: {
  ...
  if (literal === undefined) {
    throw postgresError(
      ...
      `contract convert: column "${input.namespaceId}"."${input.tableName}"."${input.columnName}" has a literal default that Prisma 8 PSL has no spelling for: the ${input.pslTypeName} type reads no PSL literal back as ${JSON.stringify(value)}.`,
      {
