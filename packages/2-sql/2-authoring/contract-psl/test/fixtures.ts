...
const genRandomUuidSig: FuncCallSig = {
  documentation: 'Uses a random UUID generated by the database (`gen_random_uuid()`) as the default value.',
};
  ...
  return {
    defaultFunctionRegistry: new Map<string, ControlMutationDefaultEntry>([
      ...
      [
        'gen_random_uuid',
        {
          signature: genRandomUuidSig,
          lower: () => ({
            ok: true as const,
            value: {
              kind: 'storage' as const,
              defaultValue: { kind: 'function' as const, expression: 'gen_random_uuid()' },
            },
          }),
          usageSignatures: ['gen_random_uuid()'],
        },
      ],
      [
        'uuid',
