...
- **Contract encoding.** A column default lowers to `{ kind: 'function', expression: <canonical body> }`, the shape `dbgenerated("...")` produced, not the `ext` envelope above. Whether raw SQL defaults should later become a content-addressed payload is recorded in projects/remove-dbgenerated/deferred.md item 1.
