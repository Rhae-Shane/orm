...
| `rename-table-migration.e2e.test.ts` | **Rename a table and keep its rows**: seed `userProfile` with a unique constraint, a foreign key, an index, row-level security and a policy, then drop the model's `@@map`. Journey R1 confirms the bare plan is refused by the case-change guard and a stale `--rename` intent fails planning, then `migration plan --rename userProfile=UserProfile` plans the `renameTable` plus a rename of each constraint and index named after the old table. Journey R2 authors the same rename through `migration new --rename`, whose scaffold carries the same operations. In both, `migrate` keeps the rows, the policy and RLS; `db verify --schema-only` is clean; a plan with no schema change is empty; and a later migration that removes the unique constraint, the foreign key and the index applies. Journey R5 renames the table in the same migration that points its foreign key at another table under an explicit name: the foreign key is renamed after the new table, then dropped and created, the rows stay and `db verify --schema-only` is clean |
| `rename-table-migration.sqlite.e2e.test.ts` | The SQLite twin of the first rename journey, driven through a file database and the SQLite facade config: the rename plus a drop and a create of each index named after the old table, then the same empty plan and removal migration. A second journey follows the by-hand path of a `db update` project: the case guard gives the two-statement rename through `_prisma_rename_UserProfile`, and after running it `db update` drops and recreates the indexes named after the old table, keeps the rows and verifies clean |
